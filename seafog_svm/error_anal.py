...
def plot_error(fig_path, error_infos):
    ...
    for error_info in error_infos:
        ...
        total['month_list'][dt.month] += 1
        total['time_list'][dt.hour * 2 + dt.minute / 30] += 1
        if error_info['error'] == 1:
            error['month_list'][dt.month] += 1
            error['time_list'][dt.hour * 2 + dt.minute / 30] += 1
    ...
    x_axis_labels = ['{}:{:0>2d}'.format(i//2, 30 if (i % 2 == 1) else 0) for i in range(48)]
    ...
    img_path = os.path.join(cfg.MAIN_PATH, cfg.DATA_PATH, 'img', 'area.png')
    real_img = cv2.imread(img_path)
    SPACE_R = 0.025
    SIZE = 1080
    LON_START, LAT_START = 105, 45
    for la, lo in zip(error['lat'], error['lon']):
        x = int((lo - LON_START) / SPACE_R)
        y = int((LAT_START - la) / SPACE_R)
        real_img[x, y, 2] = 255
        real_img[x, y, 1] = 0
        real_img[x, y, 0] = 0
...
def main():
    params_fp = os.path.join(cfg.MAIN_PATH, 'parameters.csv')
    ...
    for range_dic, sldn_dic in zip(range_list, sldn_list):
        ...
        error_file_name = os.path.join(cfg.MAIN_PATH, cfg.OUTPUT_PATH, cfg.DATA_PREFIX + 'error' + name_tag + '.csv')
        error = get_error_data(error_file_name)
        
        fig_path = os.path.join(cfg.MAIN_PATH, cfg.OUTPUT_PATH, cfg.DATA_PREFIX + 'error' + name_tag)
